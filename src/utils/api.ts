...
class BlobAPI {
    ...
    private async request<T>(
        ...
        if (!data.success) {
            ...
            throw new Error(
                `API Error: ${data.status} - ${data.data ?? "No error details provided"
                }`,
            );
        }
    ...
    async createAccessor(
        request: AccessorCreateRequest,
    ): Promise<{ accessor: DbAccessor; token: string }> {
    ...
    async signTokenForAccessor(
        accessorId: Id,
    ): Promise<{ accessor: DbAccessor; token: string }> {
        return this.request<{ accessor: DbAccessor; token: string }>(
            `/accessors/sign-token/${accessorId}`,
            {
                method: "POST",
            },
        );
