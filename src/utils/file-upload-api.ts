...
export class FileUploader {
    ...
    constructor(options: UploadOptions) {
        this.options = {
            ...
            concurrentUploads: options.concurrentUploads ||
    ...
    async uploadFiles(bucketId: string, files: File[]): Promise<UploadResult[]> {
        ...
        while (queue.length > 0 || inProgress.length > 0) {
            ...
            while (
                queue.length > 0 && inProgress.length < this.options.concurrentUploads
            ) {
                const file = queue.shift()!;
                const uploadPromise = this.uploadFile(bucketId, file)
                    .then((result) => {
    ...
    private async uploadSmallFile(
        ...
        const url = `${this.options.baseUrl}/files/${bucketId}/${encodeURIComponent(file.name)
            }?total_chunks=1`;
    ...
    private async uploadLargeFile(
        ...
        const firstChunkUrl = `${this.options.baseUrl}/files/${bucketId}/${encodeURIComponent(file.name)
        ...
        if (!firstChunkResponse.ok) {
            throw new Error(
                `Failed to upload first chunk: ${firstChunkResponse.status}`,
            );
        }
        ...
        for (let i = 1; i < totalChunks; i++) {
            ...
            const chunkUrl = `${this.options.baseUrl}/files/${bucketId}/${encodeURIComponent(file.name)
