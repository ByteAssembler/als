 ...
 */
export function hasFullCategoryAccess(
 ...
 */
export function getFullyAccessibleCategories(
  permissions: DbPermissions,
): string[] {
  return permissionSets
    .filter((set) => Permissions.hasAll(permissions, ...set.permissions))
    .map((set) => set.name);
}
...
export function determineEffectiveRole(permissions: DbPermissions): string {
  ...
  const hasAllBucketAndFilePermissions =
    hasFullCategoryAccess(permissions, "Bucket") &&
    hasFullCategoryAccess(permissions, "File");
  ...
  const canWrite = activePermissions.includes(
