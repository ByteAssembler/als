...
export const permissionLabels: Record<DB_PERMISSIONS, string> = {
	...
	[DB_PERMISSIONS.CREATE_JWT_TOKENS_FOR_ACCESSOR]: "Create JWT Tokens for Accessor",
} as const;
...
export const permissionSets = [
	...
	{
		name: "Temporary URLs",
		permissions: [DB_PERMISSIONS.SIGN_TEMPORARY_URLS, DB_PERMISSIONS.READ_TEMPORARY_URLS],
	},
	{
		name: "Other",
		permissions: [DB_PERMISSIONS.MANAGE_PERMISSIONS, DB_PERMISSIONS.CREATE_JWT_TOKENS_FOR_ACCESSOR],
	},
] as const;
...
export const predefinedRoles = {
	...
	BUCKET_MANAGER: (): DbPermissions => {
		const bucketPerms = permissionSets.find((set) => set.name === "Bucket")?.permissions || [];
		const filePerms = permissionSets.find((set) => set.name === "File")?.permissions || [];
	...
	ACCESSOR_MANAGER: (): DbPermissions => {
		const accessorPerms = permissionSets.find((set) => set.name === "Accessor")?.permissions || [];
		const tempUrlPerms = permissionSets.find((set) => set.name === "Temporary URLs")?.permissions || [];
...
export const Permissions = {
	 ...
	 */
	remove: (permissions: DbPermissions, flag: DB_PERMISSIONS): DbPermissions => {
		return permissions & ~flag;
	},
	...
	getActive: (permissions: DbPermissions): DB_PERMISSIONS[] => {
		return getAllDbPermissionFlags().filter((flag) => Permissions.has(permissions, flag));
	},
	 ...
	 */
	hasAll: (permissions: DbPermissions, ...flags: DB_PERMISSIONS[]): boolean => {
	 ...
	 */
	hasAny: (permissions: DbPermissions, ...flags: DB_PERMISSIONS[]): boolean => {
	...
	describePermissions: (permissions: DbPermissions): string[] => {
		return Permissions.getActive(permissions).map((flag) => permissionLabels[flag]);
	},
	 ...
	 */
	categorize: (permissions: DbPermissions): Record<string, DB_PERMISSIONS[]> => {
		const result: Record<string, DB_PERMISSIONS[]> = {};

		permissionSets.forEach((set) => {
			const activeInSet = set.permissions.filter((flag) => Permissions.has(permissions, flag));
...
export function getAllDbPermissionFlags(): DB_PERMISSIONS[] {
	return Object.values(DB_PERMISSIONS).filter((value) => typeof value === "number") as DB_PERMISSIONS[];
}
