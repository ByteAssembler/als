		...
		<div className="w-64 bg-gray-700 text-white p-4 h-[50dvh]">
			...
			<ul>
				{sections.map((section) => (
					<li
						key={section.id}
						className={`cursor-pointer hover:bg-gray-600 p-2 rounded ${section.id === selectedSectionId ? "bg-gray-500" : ""
							}`}
...
export default function Sidebars({ selectedSiteId, onSiteSelect, selectedSectionId, onSectionSelect }: SidebarsProps) {
	...
	const fetchSites = async (): Promise<Site[]> => {
		try {
			...
			if (!response.ok) {
				throw new Error(`Fehler beim Abrufen der Sites: ${response.statusText}`);
			}
	...
	return (
		<div className="flex">
			<div className="flex flex-col">
				...
				{selectedSiteId && (
					<SectionList sections={sections} onSectionSelect={onSectionSelect} selectedSectionId={selectedSectionId} />
				)}
			</div>
			<div className="flex-1 p-4">{error && <p className="text-red-500">{error}</p>}</div>
		</div>
	);
}
